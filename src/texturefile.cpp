...
void TextureFile::EnableAlbedo(const ppl7::String &path)
{
    use_albedo = true;
    albedo_path = path;
}

void TextureFile::EnableNormal(const ppl7::String &path)
{
    use_normal = true;
    normal_path = path;
}

void TextureFile::EnableSpecular(const ppl7::String &path)
{
    use_specular = true;
    specular_path = path;
}
...
int TextureFile::AddSurface(ppl7::grafix::Drawable &surface, ppl7::grafix::Drawable &normal,
                            ppl7::grafix::Drawable &specular, ppl7::grafix::Rect *r, int id, int pivotx, int pivoty)
{
    ppl7::grafix::Rect rr;
    if (!r) {
        rr.x1 = 0;
